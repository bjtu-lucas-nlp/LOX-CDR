...
class DataLoader:
    ...
    def load_data(self, data_path, index_dir):
        ...
        reviews = pickle.load(open(data_path, 'rb'))
        for review in reviews:
            # lucas (feature, adjective, sentence, sentiment)
...
def ids2tokens(ids, tokenizer, eos):
    ...
    # lucas 240325 split ### Response text
    # text = text[text.find("### Response") + 14:]
    text = postprocessing(text)  # process punctuations: "good!" -> "good !"
    text = text.strip("[/INST]</s>").strip()
    text = text.lower()     # lucas 240422 change the text to lower text
